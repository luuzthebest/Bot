...
help_text = [
    [
        ...
        ("Examples:",
         "```<PREFIX><COMMAND> @$.LvvzMalphas..!```"
         "```<PREFIX><COMMAND> $.LvvzMalphas..!#9999\nBeing mean :(```"
         "```<PREFIX><COMMAND> lvvz\nSound board abuse```"),
    ]
]
...
async def execute(ctx, params):
    ...
    try:
        text = (
            "‼ **Votereject** ‼\n"
            "{initiator} has initiated a votereject against {offender}.{reason}\n\n"
            "{participants}:\nVote by reacting with ✅ to reject {offender}, "
        ...
        return False, "I don't have permission to reply to your reject command."
    cfg.VOTEKICKS[m.id] = {
        "initiator": author,
